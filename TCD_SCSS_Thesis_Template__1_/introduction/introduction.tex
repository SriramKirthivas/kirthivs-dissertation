...
% % \section{Research Identification Setup}

% % When conducting internet scanning, it is good practice to provide identification so that network administrators can understand the purpose of the scan and contact the researcher if needed.

% % \subsection{DuckDNS Domain Setup}

% % To provide a resolvable domain name for the scanning VPS:

% % \begin{enumerate}
% %     \item Go to \url{https://www.duckdns.org}
% %     \item Sign in with a provider (Google, GitHub, etc.)
% %     \item Create a subdomain (e.g., \texttt{tcd-sriram})
% %     \item Point it to the VPS public IP address
% % \end{enumerate}

% % Result: The VPS is now accessible via \texttt{tcd-sriram.duckdns.org}

% % \subsection{Research Identification Web Page}

% % A simple HTML page was created to identify the research purpose. When anyone accesses the VPS public IP or the DuckDNS URL, they see this information page.

% % The page includes:
% % \begin{itemize}
% %     \item Institution: Trinity College Dublin
% %     \item Researcher name and supervisor
% %     \item Research purpose: Internet scanning infrastructure for cryptographic key re-use study
% %     \item Contact email for concerns
% % \end{itemize}

% % To serve the page, a simple web server can be started:
...
% % \section{ZMap Scanning Tests}

% % \subsection{Verifying Port 25 Connectivity}

% % Before scanning Irish IP ranges, verify that outbound port 25 is working by testing against a known mail server:

% % \begin{verbatim}
% % # Get Google's mail server IP
% % host gmail-smtp-in.l.google.com

% % # Test with ZMap
% % sudo zmap -p 25 142.250.153.26/32 -o test-gmail.ips
% % cat test-gmail.ips
% % \end{verbatim}

% % Expected output shows 100\% hitrate:
% % \begin{verbatim}
% % 0:02 25%; send: 1 done; recv: 1; hitrate: 100.00%
% % \end{verbatim}

% % This confirms ZMap and port 25 are working correctly.

% % \subsection{Test Scan with Sample Irish IP Ranges}

% % A test whitelist file was created with a small subset of Irish IP ranges:
...
% % Total IPs in test file: approximately 8,512 IPs.

% % \subsection{Running the Test Scan}

% % \begin{verbatim}
% % sudo zmap -B 100K -p 25 --whitelist-file=test-ips.IE.v4 \
% %     --blacklist-file=blacklist.conf -o test-zmap.ips
% % \end{verbatim}

% % \subsection{Test Scan Output}

% % \begin{verbatim}
% % 0:00 0%; send: 0 (0 p/s avg); recv: 0; hitrate: 0.00%
% % 0:10 15%; send: 1474 (147 p/s avg); recv: 0; hitrate: 0.00%
% % 0:20 30%; send: 2948 (147 p/s avg); recv: 0; hitrate: 0.00%
% % 0:30 46%; send: 4420 (147 p/s avg); recv: 0; hitrate: 0.00%
% % 0:40 61%; send: 5894 (147 p/s avg); recv: 0; hitrate: 0.00%
% % 0:50 76%; send: 7365 (147 p/s avg); recv: 0; hitrate: 0.00%
% % 0:58 88%; send: 8512 done (147 p/s avg); recv: 0; hitrate: 0.00%
% % \end{verbatim}
...
% % \hline
% % Total IPs scanned & 8,512 \\
% % \hline
% % Scan duration & ~1 minute \\
% % \hline
% % Average rate & 147 packets/sec \\
% % \hline
...
% % The 0\% hitrate is expected because the test IP ranges are primarily CDN and cloud provider ranges (Akamai, AWS, Azure) which are geolocated to Ireland but do not typically run public SMTP servers:

% % \begin{itemize}
% %     \item 1.x.x.x - APNIC (Asia-Pacific region)
% %     \item 2.x.x.x - Akamai CDN
% %     \item 3.x.x.x - AWS cloud infrastructure
% % \end{itemize}

% % The test confirms that:
% % \begin{enumerate}
% %     \item ZMap is functioning correctly (verified with Google mail server test)
% %     \item Port 25 outbound is enabled on the VPS
% %     \item Rate limiting (-B 100K) is working as expected
% %     \item The scanning infrastructure is ready for production use
% % \end{enumerate}

% % For actual SMTP server discovery, the full \texttt{mm-ips.IE.v4} file should be used, which includes traditional Irish ISP ranges (77.x, 86.x, 87.x, 89.x, etc.) where mail servers are more likely to be found.
...
This chapter provides an overview of the project, including its motivation, objectives, and structure. It sets the context for the research on cryptographic key reuse and outlines the significance of the study in understanding security practices on the Internet.

\section{Motivation}

Cryptographic keys are what keep internet connections private. Every SSH login and every TLS-secured email depends on the assumption that each server has its own unique key. But in practice, keys get reused across multiple servers, whether through cloned VM images, shared hosting templates, or simple misconfiguration. If an attacker compromises one server's private key, they can impersonate every other server that shares it.

Prof. Farrell's earlier work \cite{cryptoeprint:2018/299} showed this is not a theoretical concern. A 2018 scan of Irish mail servers found 1,437 clusters of hosts sharing keys, with the largest cluster containing nearly 2,000 hosts. Since then, cloud adoption has grown, Let's Encrypt has become the dominant certificate authority, and hosting practices have changed considerably. The question is whether key reuse has actually gotten better or whether it has just shifted into different patterns.

That is what motivated this project. I wanted to re-run the same kind of scan, using the same core methodology but with updated tools and data, and see what the Irish internet looks like now. Repeating a measurement study like this is valuable because single snapshots only tell you so much. Comparing results across years is what lets you spot trends and say whether things are actually improving.

\section{Objectives}

This project has both project objectives and personal objectives. The project objectives focus on the research and technical aspects of investigating cryptographic key reuse, while the personal objectives aim to enhance my skills and knowledge in relevant areas.

\subsection{Project Objectives}

The primary objective of this project is to investigate how common cryptographic key reuse is among Irish mail servers in 2026. Based on the previous research done on this project, the project aims to:
...
\begin{itemize}
    \item Gain a deeper understanding of cryptographic protocols and their security implications.
    \item Gain hands-on experience with network scanning tools like ZMap and ZGrab.
    \item Develop skills in large-scale data collection and analysis.
    \item Enhance proficiency in shell scripting and Python for data processing.
    \item Get more comfortable with Git workflows, especially working with forks and pull requests.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Chapter 2 - Background:} Provides background information on the protocols involved, including email architecture, TLS, SSH, STARTTLS, certificate infrastructure, and the scanning tools used.
    \item \textbf{Chapter 3 - Experimental Methods:} Describes the implementation details of the project, including the methodology used for data collection and analysis.
    \item \textbf{Chapter 4 - Results and Discussion:} Presents the results of the scan, including cluster statistics and visualisations, with comparisons to previous years.
    \item \textbf{Chapter 5 - Evaluation:} Evaluates the findings and discusses their significance.
    \item \textbf{Chapter 6 - Conclusions and Future Work:} Summarises the key findings and suggests possible directions for future work.
\end{itemize}